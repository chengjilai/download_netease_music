...
if not cd:
    ...
    if r.status != 200 or not r.body or r.body.get("code") != 200:
        ...
        print(f"Login failed: {msg or f'status={r.status}'}"); sys.exit(1)
...
for lv in ["exhigh", "higher", "standard"]:
    ...
    if d and d[0].get("url"):
        sd = d[0]
        print(f"  {sd.get('type')} @ {sd.get('br')}bps, {sd.get('size',0)/1024/1024:.1f}MB")
        break
else:
    print("No playable URL (VIP-only or geo-locked)."); sys.exit()
...
subprocess.run(
    ["curl", "-s", "-4", "-L", "--connect-timeout", "10", "--max-time", "300",
     "-H", "User-Agent: Mozilla/5.0", "-o", path, sd["url"]],
...
subprocess.run(
    ["ffprobe", "-hide_banner",
     ...
     "-of", "default=noprint_wrappers=1", path], timeout=10)
print(f"Ready: {path}")
